...
public class SingleTigerSession extends Object {
  ...
  /**
   * 'languageNameSets' are in direct corelation with
   * 'languageFiles' and 'languageProperties', do not change their order.
   */
  ...
  static {
    ...
    try { initialLoad(); } catch (Throwable t) { }
    try { initialAvailability(); } catch (Throwable t) { }
  }
  ...
  private static void initialAvailability() {
    ...
    for (int i=0; i<languageFiles.length; i++) {
      ...
      if (available) languageNamesAvailableV.addElement(languageNames[i]);
    }
  ...
  /**
   * @return true iff the exact language specified was found, else (or if default was loaded) return false;
   */
