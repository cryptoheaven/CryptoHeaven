 ...
 * Class Description:
 *
 * Parse words from a Swing JTextComponent. In addition to parsing, this class
 * can also delete words, replace words, and detect doubled words.
 ...
 * Class Details:
 *
 * <P>The text in the JTextComponent is read when the TextCompWordParser is
 * constructed. If the contents of the JTextComponent change, the updateText
 * method must be called.
 *
 * <P>If the JTextComponent can contain text with HTML markups, extend this
 * class from HTMLStringWordParser instead of StringWordParser.
 *
 * <b>$Revision: 1.3 $</b>
 *
...
public class TigerWordParser extends StringWordParser {
   ...
   * @param isHyphenDelim true if a hyphen is to be considered a word delimiter,
   * false if it's part of a word
   * @param selectCurWord true if each word should be selected in the component
   * to show context. Enabling this option degrades performance.
   */
  ...
  /**
   * Delete a specified number of characters from the text starting at the
   * current cursor position.
   *
   * @param numChars The number of characters to delete.
   * @exception NoSuchElementException Attempt to delete beyond the end of the
   * string.
   */
   ...
   * @exception NoSuchElementException The cursor is positioned at the end of
   * the string.
   */
  ...
  /**
   * Report that the contents of the TextArea have changed. This method must be
   * called if the TextArea is edited by the user or updated by other software.
   * Currently, it causes checking of the TextArea to restart. In future, it
   * will call the 1.1 TextArea's getCaret method and check from that point.
   */
  ...
  /**
   * true if the current word should always be selected in the TextArea to show
   * context.
   */
