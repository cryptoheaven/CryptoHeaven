...
public class Compress extends SCSU {
   ...
   **/
  private boolean positionWindow(int ch, byte [] out, boolean fUnicodeMode) throws IllegalInputException, EndOfOutputException {
  ...
  public byte[] compress(String inStr) throws IllegalInputException, EndOfInputException {
    return compress(inStr.toCharArray());
  }
  public byte[] compress(char[] inChars) throws IllegalInputException, EndOfInputException {
    ...
    byte [] out = new byte[inChars.length * 2];
    char [] in = inChars;
    reset();
    while(true) {
      try {
