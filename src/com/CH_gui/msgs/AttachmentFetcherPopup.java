...
public class AttachmentFetcherPopup extends ThreadTraced {
  ...
  private void updatePopup(final Record[] attachments) {
    ...
    for (int i=0; i<msgLinks.length; i++) {
      String subject = ListRenderer.getRenderedText(msgLinks[i], true, false, false, false, false, true, true, null);
    ...
    for (int i=0; i<fileLinks.length; i++) {
      JMenuItem fileItem = new JMyMenuItem(ListRenderer.getRenderedText(fileLinks[i], true, false, false, false, false, true, true, null), ListRenderer.getRenderedIcon(fileLinks[i]));
